...
| XA ADPCM | One exhaustive coding decoder accepts the 16 coding-field-valid bytes across emphasis, 4/8-bit, mono/stereo, and 18.9/37.8 kHz; reserved encodings are rejected before buffer allocation. The established predictor is unchanged. | Standards, S, T (`[cdic][xa][coding]`, all 256 bytes). | De-emphasis, sound-group copy signaling, independent PCM vectors, and silicon rounding captures are missing. |
