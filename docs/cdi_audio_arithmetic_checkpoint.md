...
The new deterministic candidate tests establish:
...
The floating-point de-emphasis path already required a deterministic conversion back to signed PCM16. That boundary is now centralized as:
...
The XA decoder remains on its independently corroborated fixed-point compatibility model: Green Book predictor coefficients scaled by 256, `+128` before the eight-bit arithmetic shift, followed by final PCM16 saturation. New adversarial tests pin previously implicit boundary behavior:
...
- over-range predictor results saturate before being stored back into the predictor history.

These tests make the existing FFmpeg-correlated model portable and reviewable. They do not identify the IMS66490/CDIC accumulator width or prove that physical silicon rounds at the same internal stage.

## Regression additions

`tests/emu/philips/cdi_audio_arithmetic.cpp` adds three focused cases:
