...
// Preserve the validated idle IRQ4 acknowledge behavior.
static constexpr uint8_t IRQ4_IDLE_COMPAT_VECTOR = 0x3c;
...
{
	map(cdi_mono2::PLANE_A_START, cdi_mono2::PLANE_A_END).ram().share("plane0");
	map(cdi_mono2::PLANE_B_START, cdi_mono2::PLANE_B_END).ram().share("plane1");
#if ENABLE_UART_PRINTING
	map(0x301400, 0x301403).r(m_maincpu, FUNC(scc68070_device::uart_loopback_enable));
#endif
...
static INPUT_PORTS_START( cdi )
	PORT_START("MOUSEX")
	PORT_BIT(0xffff, 0x000, IPT_MOUSE_X) PORT_SENSITIVITY(100) PORT_KEYDELTA(2) PORT_CODE_DEC(KEYCODE_LEFT) PORT_CODE_INC(KEYCODE_RIGHT)

	PORT_START("MOUSEY")
	PORT_BIT(0xffff, 0x000, IPT_MOUSE_Y) PORT_SENSITIVITY(100) PORT_KEYDELTA(2) PORT_CODE_DEC(KEYCODE_UP) PORT_CODE_INC(KEYCODE_DOWN)
...
void cdi_state::update_irq4()
{
	...
	m_maincpu->in4_w(
		m_irq4_owner != IRQ4_NONE ? ASSERT_LINE : CLEAR_LINE);
}
