...
void m68000_musashi_device::x4e73_rte_l_71()
{
	if(m_s_flag) {
		...
		if(format_word == 0) {
			...
			else if (format_word == 0x0f && CPU_TYPE_IS_070()) /* SCC68070 - 17-word format-F frame */
			{
				new_sr = m68ki_pull_16();
				new_pc = m68ki_pull_32();
				m68ki_fake_pull_16();  /* format/vector word */
				m68ki_fake_pull_16();  /* special status word */
				m68ki_fake_pull_16();  /* current move multiple mask */
				m68ki_fake_pull_32();  /* internal information */
				m68ki_fake_pull_32();  /* data output buffer */
				m68ki_fake_pull_32();  /* fault address */
				m68ki_fake_pull_32();  /* data input buffer */
				m68ki_fake_pull_16();  /* instruction register */
				m68ki_fake_pull_16();  /* instruction input buffer */
				m68ki_fake_pull_16();  /* internal information */
				m68ki_jump(new_pc);
				m68ki_set_sr(new_sr);
				m_instr_mode = INSTRUCTION_YES;
				m_run_mode = RUN_MODE_NORMAL;
