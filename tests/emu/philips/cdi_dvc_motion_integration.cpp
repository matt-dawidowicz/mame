...
class cdi_motion_state : public cdi_state
{
  public:
	...
	void capture(std::map<std::string, std::vector<std::pair<const float *, int>>> const &sound)
	{
		...
		if (!long_seconds && !m_pass && end > int64_t(m_baseline_pcm.size()))
		...
		for (int i = 0; i < count; ++i)
		{
			...
			int64_t const relative = pos - 300 * 44100 / 1000;
			bool const active = relative >= 0 && (long_seconds || relative < 2 * 112896);
  ...
  private:
	...
	void display_setup(address_space &space)
	{
		...
		command(0xc0040001); // CLUT8 A + external video; B disabled
	...
	void video_chunk(unsigned profile, int pts, unsigned stream)
	{
		auto const &bytes = video[profile];
		for (unsigned offset = 0; offset < bytes.size(); offset += 16000)
			packet(false, bytes.data() + offset, std::min<unsigned>(16000, bytes.size() - offset),
				   offset == 0 ? pts : -1, stream);
	}
	...
	void begin_scene(address_space &space)
	{
		...
		auto const &p = cdi_motion_reference::PROFILES[profile];
		...
		space.write_word(0xe03008, m_branch);
		space.write_word(0xe040c0, 0x0100); // clear decoder before format branch
		space.write_word(0xe040c4, m_branch);
		...
		if (m_branch)
		{
			...
			video_chunk(initial_profile, -1, 0);
			packet(true, cdi_av_reference::AUDIO.data(), 626, -1, 0);
			expect(space.read_word(0xe0300a) == 0, "current audio stream changed before requested header");
		}
		video_chunk(profile, m_branch ? 0 : 27000, m_branch);
		audio_chunk(m_branch ? 0 : 27000, m_branch);
		space.write_word(0xe040c0, 0x0008);
		expect(space.read_word(0xe0300a) == m_branch, "requested audio stream did not commit");
		m_last_frame = -1;
	}
	uint32_t render(screen_device &screen, bitmap_rgb32 &bitmap, rectangle const &clip)
	{
		...
		if (clip.min_y <= top && clip.max_y >= top)
		{
			...
			auto const &p = cdi_motion_reference::PROFILES[m_field_profile];
			int64_t const elapsed = m_field_time - (m_branch ? int64_t(m_branch_ms) * 90 : 27000);
		...
		for (int y = top; y <= bottom; ++y)
			for (int x = 0; x < 768; ++x)
			{
				...
				uint32_t wanted = x < 96 ? 0x4386c9 : 0;
				int const sy = y - top;
				bool decoded = false;
				...
				for (unsigned shift : {0, 8, 16})
				{
					...
					frame_error = std::max(frame_error, error);
					if (decoded)
					{
		...
		expect(frame_error <= 12, "composed RGB mismatch t=" + std::to_string(m_field_time) + " frame=" +
									  std::to_string(m_field_frame) + " error=" + std::to_string(frame_error));
		if (!long_seconds)
		{
			if (!m_pass)
	...
	TIMER_CALLBACK_MEMBER(step)
	{
		...
		if (m_ms == (long_seconds ? 300 + long_seconds * 1000 : 4400))
		{
			...
			if (long_seconds || m_pass == snapshots.size())
			{
				completed = true;
				machine().schedule_exit();
			}
	...
	std::vector<unsigned> m_saved_ms, m_saved_chunks;
...
void run_cdi_motion(unsigned profile, unsigned seconds)
{
	cdi_q_disc temp;
	emu_options options;
	...
	if (!seconds)
	{
		...
		CHECK(state.restored_fields > 100);
		CHECK(state.restored_samples > 300000);
	}
	else
	{
