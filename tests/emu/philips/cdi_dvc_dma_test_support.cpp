...
#if defined(CDI_CERTIFICATION_TESTS)
...
{
	cdi_transport_sound_hook(sound);
	cdi_cdda_save_sound_hook(sound);
#if defined(CDI_CERTIFICATION_TESTS)
	cdi_decoded_av_sound_hook(sound);
	cdi_motion_sound_hook(sound);
#endif
}
